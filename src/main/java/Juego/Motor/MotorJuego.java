...
public class MotorJuego {
    ...
    private void ejecutarUsar(int indiceObjeto){
        ...
        // Impedir usar objetos que ya están equipados
        if (!objeto.esEquipable() && !objeto.esConsumible()) {
            registrar("No puedes usar " + objeto.getNombre() + ". Ya está equipado o no se puede usar.");
            return;
        }
    ...
    private void ejecutarTurnoEnemigo(Enemigo enemigo){
        ...
        if (!mejorPos.equals(enemigo.getPosicion())) { //Si existe la mejor posicion y no esta ya en ella
            Posicion anterior= enemigo.getPosicion();
            hab.getCelda(anterior).setEnemigo(null);
            enemigo.setPosicion(mejorPos);
            hab.getCelda(mejorPos).setEnemigo(enemigo);
